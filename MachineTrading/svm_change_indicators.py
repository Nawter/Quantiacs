...
def predict(OPEN, HIGH, LOW, CLOSE, VOL, settings):
    ...
    data_temp['sign_close_diff'] = np.where(abs(close_open_diff) > fee, np.sign(close_open_diff),
                                            0.0)  # replace inf values with nan values
    ...
    # settings['X_train'] = pd.concat([X_train, settings['X_train']], ignore_index=True, sort=True)
    # settings['X_test'] = pd.concat([X_test, settings['X_test']], ignore_index=True, sort=True)
